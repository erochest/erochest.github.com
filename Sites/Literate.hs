...
instance Monoid RunBlock where
        ...
        mappend a@(LCodeBlock _) b@(LTextBlock _) = BlockRun [a, b]
        mappend a@(LTextBlock _) b@(LCodeBlock _) = BlockRun [a, b]
...
illiterateLine _ Start ('#':'!':_) = (ActiveText, mempty)
...
clojure :: Item String -> Item String
clojure = markdown . fmap (illiterate "clojure" [ CommentLine "; "
                                                , CommentLine ";"
                                                ])

markdown :: Item String -> Item String
markdown = writePandoc . fmap (readMarkdown defaultHakyllReaderOptions)
