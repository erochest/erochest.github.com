...
For this, we’ll need a number of dependencies, so we’ll list these in our [Leiningen](http://leiningen.org/) [`project.clj`](project.clj) file.
...
And for the data file, we can just use a sample of the Stack Exchange comments. You can find a (non-random) sample of 100,000 lines from that file in the source code for this chapter or download it from [here](/clj-data-analysis/data/comments.xml).
...
Next, we’ll add some general data parsing functions. The first is simply a lazy version of `clojure.string/split-lines`. The rest are more specifically targeted to the task at hand. They identify lines from the input that contain data, parse those into map, and eventually parse them into a UserPost.
...
The next two functions process a sequence of lines into a map, which associates the users’ identifiers with the `UserPost` objects for each user.
...
Now we compare two methods’ performance. (Measured with the [criterium](https://github.com/hugoduncan/criterium) library.)
